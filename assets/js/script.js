...
function atualizarprofile(index, userData) {
    ...
    nameElement.textContent =
        userData.user.globalName || userData.user.username || " ";
    ...
    const flags = {
        ...
        HypeSquadOnlineHouse1: "<img class='flag-icon' src='https://raw.githubusercontent.com/irrecusavel/discord/8e35b50a8186eb92f54bb5446e7184ddc088aa4e/discord/hypebravery.svg' alt='HypeSquad Online House 1' title='Bravery do HypeSquad'>",
        HypeSquadOnlineHouse2: "<img class='flag-icon' src='https://raw.githubusercontent.com/irrecusavel/discord/8e35b50a8186eb92f54bb5446e7184ddc088aa4e/discord/hypebrilliance.svg' alt='HypeSquad Online House 2' title='Brilliance do HypeSquad'>",
        HypeSquadOnlineHouse3: "<img class='flag-icon' src='https://raw.githubusercontent.com/irrecusavel/discord/8e35b50a8186eb92f54bb5446e7184ddc088aa4e/discord/hypebalance.svg' alt='HypeSquad Online House 3' title='Balance do HypeSquad'>",
		...
		QuestCompleted: "<img class='flag-icon' title='Completou uma Missão' src='https://cdn.discordapp.com/badge-icons/7d9ae358c8c5e118768335dbe68b4fb8.png'>",
    };

flagsElement.innerHTML = userData.profile.badgesArray && userData.profile.badgesArray.length > 0 ?
    userData.profile.badgesArray
    .map((flag) => {
...
const usersToAddLegacyUsername = ["376389353947136022"];
if (usersToAddLegacyUsername.includes(userData.user.id)) {
    const LegacyUsernameHtml = `<img class='flag-icon' src='https://raw.githubusercontent.com/irrecusavel/discord/8e35b50a8186eb92f54bb5446e7184ddc088aa4e/discord/pomelo.svg' alt='Legacy Username Badge' title='Originalmente ${userData.user.legacyUsername}'>`;
    flagsElement.innerHTML += `<div class="tooltip" style="white-space: nowrap;">${LegacyUsernameHtml}<span class="tooltiptext">Originalmente ${userData.user.legacyUsername}</span></div>`;
}

const usersToAddQuestCompletedBadge = ["376389353947136022"];
if (usersToAddQuestCompletedBadge.includes(userData.user.id)) {
    const questCompletedHtml = `<img class='flag-icon' src='https://cdn.discordapp.com/badge-icons/7d9ae358c8c5e118768335dbe68b4fb8.png' alt='Completou uma Missão' title='Completou uma Missão'>`;
    flagsElement.innerHTML += `<div class="tooltip" style="white-space: nowrap;">${questCompletedHtml}<span class="tooltiptext">Completou uma Missão</span></div>`;
}
    ...
    const connections = {
        ...
        discord: {
            icon: "<img class='conn-icon' src='https://cdn.discordapp.com/emojis/1233092927639781527.webp?size=128&quality=lossless'>",
            link: "https://discord.gg/wvzB2QDZnB",
            user: true,
        }
    };
    ...
    connsElement.innerHTML =
        ...
        .map((conn) => {
            ...
            if (lowerCaseType in connections) {
                ...
                if (connection.off) {
                    return `<a title="${conn.name || ""}" class="tooltip">${connection.icon
                }<span class="tooltiptext">${conn.name || ""}</span></a>`;
                }
                if (connection.user) {
                    return `<a href="${connection.link}${conn.name
                }" target="_blank" class="tooltip">${connection.icon
                }<span class="tooltiptext">${conn.name || ""}</span></a>`;
                } else {
                    return `<a href="${connection.link}${conn.id
                }" target="_blank" class="tooltip">${connection.icon
                }<span class="tooltiptext">${conn.name || ""}</span></a>`;
                }
            }
    ...
    if (
        userData.user.id === "376389353947136022" ||
        userData.user.id === "376389353947136022"
    ) {
        const discordConnection = connections["discord"];
        if (discordConnection) {
            const discordHtml = `<a href="${discordConnection.link}" target="_blank" class="tooltip"
      style="margin-left: 9px;">
                  ${discordConnection.icon}
                  <span class="tooltiptext">Utopia</span></a>`;
            connsElement.innerHTML += discordHtml;
        }
    }
